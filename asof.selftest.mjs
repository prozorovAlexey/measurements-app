...
await step('каталог содержит 22 замера и все поля этапа 2', () => {
  ...
  assert.deepEqual(
    ...
    [
      'По тазовым костям — нащупываются, если встать руки в боки: это именно те косточки, в которые упираются ладони. Лента идёт строго горизонтально через них, обычно на 10–15 см ниже талии. Не путать с «Талия (WHO)» (там мерят между рёбрами и тазом, выше) и с «Бёдра» (там мерят ниже, по ягодицам)',
      'Стоя, стопы вместе'
    ]
  );
  assert.deepEqual(
    ...
    [
      'По косточке на запястье, которая торчит сбоку — ближе к мизинцу, если смотреть на тыльную сторону ладони',
      'Рука расслаблена'
    ]
  );
  assert.deepEqual(
